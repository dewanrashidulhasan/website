        ...
        {cartData.map((item, index) => {
          ...
          return (
            <div key={index} className='py-4 border-t border-b text-gray-700 grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-60 flex'>
              ...
              <input className='border max-w-10 sm:max-w-20 px-1  py-1' type="number" min={1} defaultValue={item.quantity}/>
              <img className='w-4 mr-4 sm:w-5 corsor-pointer' src={assets.bin_icon} alt="" />
            </div>
          );
        })}
      </div>
